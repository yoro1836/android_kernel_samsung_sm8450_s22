...
int cam_ois_init(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	// GGFADE register(0x5E)
    rc = cam_ois_i2c_write(o_ctrl, GGFADE, 0x32,
        CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
    if (rc < 0)
        CAM_ERR(CAM_OIS, "ois gain setting GGFADE i2c write fail %d", rc);
    else
        CAM_ERR(CAM_OIS, "ois gain setting GGFADE i2c write success %d", rc);
#endif	
