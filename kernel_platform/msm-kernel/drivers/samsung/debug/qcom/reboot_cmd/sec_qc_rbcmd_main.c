		...
		struct sec_reboot_param *param)
{
	...
	raw_notifier_call_chain(&pon_rr_writers, pon_rr, param);
}
...
static RAW_NOTIFIER_HEAD(sec_rr_writers);

int sec_qc_rbcmd_register_sec_rr_writer(struct notifier_block *nb)
{
	int err;

	err = raw_notifier_chain_register(&sec_rr_writers, nb);
	if (err)
		return err;
...
{
	raw_notifier_chain_unregister(&sec_rr_writers, nb);
}
		...
		struct sec_reboot_param *param)
{
	...
	raw_notifier_call_chain(&sec_rr_writers, sec_rr, param);
}
